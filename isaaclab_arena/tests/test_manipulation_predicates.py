...
def _test_released(_simulation_app) -> bool:
    ...
    for device in ("cpu", "cuda:0"):
        for dtype in (torch.float32, torch.float64):
            ...
            result = released(env, hand, stall_threshold=0.001953125, grasp_width_tolerance=0.00390625)
            assert result.tolist() == [True, False, True, True, True, True]
            assert result.device == measured.device and result.dtype == torch.bool
            assert released(env, hand, stall_threshold=0.02, grasp_width_tolerance=0.00390625).all()
            assert not released(env, hand, grasp_width_tolerance=0.01)[-1]
            assert released(env, hand).tolist() == [True, False, True, True, False, True]
    return True
...
def _test_withdrawn(_simulation_app) -> bool:
    ...
    for device in ("cpu", "cuda:0"):
        for dtype in (torch.float32, torch.float64):
            ...
            result = withdrawn(env, "object", hand, tcp_distance_min=0.25)
            ...
            hand["tcp_offset_xyz"] = (0.125, 0.0, 0.0)
            ...
            assert withdrawn(env, "object", hand, tcp_distance_min=0.0625).tolist() == [False, True, True]
            hand["tcp_offset_xyz"] = (0.0, 0.0, 0.0)
    return True
