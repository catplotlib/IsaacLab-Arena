    ...
    try:
        for attempt in range(attempts):
            ...
            for env_id in released:
                reasons = _passive_drift(snapshot, env, env_id, passive_keys, params)
...
def _passive_drift(
    snapshot: SceneSnapshot, env: ManagerBasedEnv, env_id: int, passive_keys: list[str], params: ClutterSettleParams
) -> list[str]:
    """Report passive objects or robot links that moved beyond the permitted tolerances."""
    reasons = []
    for key in passive_keys:
        drift = _pose_drift_reason(snapshot.poses[key][env_id], env.arena_world.get_pose_e(key)[env_id], params)
        if drift:
            reasons.append(f"{key}: {drift}")
    for key, initial in snapshot.links.items():
...
def _pose_drift_reason(initial: torch.Tensor, current: torch.Tensor, params: ClutterSettleParams) -> str | None:
    ...
    reason = pose_drift_reason(initial, current, params.passive_move_thresh_m, params.passive_turn_thresh_deg)
    return f"passive drift: {reason}" if reason is not None else None
