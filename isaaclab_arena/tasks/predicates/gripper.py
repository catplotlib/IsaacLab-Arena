...
def released(
    ...
    gripped = ((measured - commanded) > stall_threshold) & (torch.abs(gap - grasp_width_m) < grasp_width_tolerance)
    return ~gripped
...
def withdrawn(
    ...
    tcp_position_B = t_W_B.new_tensor(tcp_offset_xyz).expand_as(t_W_B)
