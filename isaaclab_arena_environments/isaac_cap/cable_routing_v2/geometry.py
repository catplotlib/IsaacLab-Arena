...
"""Ordered cable winding geometry for cable-routing tasks."""
...
def capsule_centerline(body_poses_xyzw: torch.Tensor, half_lengths: torch.Tensor) -> torch.Tensor:
    """Return each capsule's start followed by the final end, as Berkeley scores it."""
    centers = body_poses_xyzw[..., :3]
    x, y, z, w = body_poses_xyzw[..., 3:7].unbind(dim=-1)
    axes = torch.stack(
        (2 * (x * z + y * w), 2 * (y * z - x * w), 1 - 2 * (x * x + y * y)),
        dim=-1,
    )
    offsets = axes * half_lengths[..., None]
    return torch.cat((centers - offsets, centers[..., -1:, :] + offsets[..., -1:, :]), dim=-2)
...
def cable_route_success_from_geometry(
    ...
    *,
    ...
    route_peg_indices_tensor = (
        torch.tensor(route_peg_indices, device=cable_points_w.device).unsqueeze(0).expand(cable_points_w.shape[0], -1)
    )
    route_directions_tensor = (
        torch.tensor(
            route_directions,
        ...
        .unsqueeze(0)
        .expand(cable_points_w.shape[0], -1)
    )
