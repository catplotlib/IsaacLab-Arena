...
class ContinuousJointPositionZeroToOneAction(BinaryJointPositionAction):
    """Map zero-to-one commands continuously between configured joint limits."""

    def process_actions(self, actions: torch.Tensor) -> None:
        """Clamp finite commands and interpolate the open and closed targets."""
        closedness = torch.nan_to_num(actions, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
        self._raw_actions[:] = closedness
        self._processed_actions = self._open_command + closedness * (self._close_command - self._open_command)
...
@configclass
class ContinuousJointPositionZeroToOneActionCfg(BinaryJointPositionActionCfg):
    """Configure a continuous zero-open, one-closed joint action."""

    class_type: type[ContinuousJointPositionZeroToOneAction] = ContinuousJointPositionZeroToOneAction
